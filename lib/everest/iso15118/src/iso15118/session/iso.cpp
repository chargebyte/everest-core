...
namespace {
void clamp_dc_limits(const d20::DcTransferLimits& upper_bound, d20::DcTransferLimits& candidate) {
    namespace dt = message_20::datatypes;

    const auto clamp_rational = [](const dt::RationalNumber& bound, dt::RationalNumber& value) {
        if (dt::from_RationalNumber(value) > dt::from_RationalNumber(bound)) {
            value = bound;
        }
    };

    clamp_rational(upper_bound.voltage.max, candidate.voltage.max);
    clamp_rational(upper_bound.charge_limits.current.max, candidate.charge_limits.current.max);
    clamp_rational(upper_bound.charge_limits.power.max, candidate.charge_limits.power.max);
    clamp_rational(upper_bound.voltage.min, candidate.voltage.min);
    clamp_rational(upper_bound.charge_limits.current.min, candidate.charge_limits.current.min);
    clamp_rational(upper_bound.charge_limits.power.min, candidate.charge_limits.power.min);

    if (upper_bound.discharge_limits.has_value() and candidate.discharge_limits.has_value()) {
        clamp_rational(upper_bound.discharge_limits->current.max, candidate.discharge_limits->current.max);
        clamp_rational(upper_bound.discharge_limits->power.max, candidate.discharge_limits->power.max);
        clamp_rational(upper_bound.discharge_limits->current.min, candidate.discharge_limits->current.min);
        clamp_rational(upper_bound.discharge_limits->power.min, candidate.discharge_limits->power.min);
    }
}
} // namespace
...
TimePoint const& Session::poll() {
    ...
    if (message_exchange.has_response()) {
        const auto [payload_type, response_type] = message_exchange.peek_response_meta();
        ...
        if (response_send_after.has_value() && now < response_send_after.value()) {
            ...
            send_response(payload_type, response_type);
        }
    } else {
        response_send_after.reset();
    }
...
void Session::send_response(io::v2gtp::PayloadType payload_type, message_20::Type response_type) {
    const auto [got_response, payload_size, stored_payload_type, stored_response_type] =
        message_exchange.check_and_clear_response();
    if (not got_response) {
        return;
    }
    payload_type = stored_payload_type;
    response_type = stored_response_type;
