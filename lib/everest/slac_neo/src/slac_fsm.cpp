...
namespace {
struct FsmTelemetryState {
    std::vector<std::string> states;
    std::map<std::string, FsmTelemetryState> submachines;
    std::vector<FsmTelemetryState> sessions;
};

nlohmann::json build_fsm_state_json(FsmTelemetryState const& state) {
    nlohmann::json json = nlohmann::json::object();

    if (!state.states.empty()) {
        json["states"] = state.states;
    }
    if (!state.submachines.empty()) {
        nlohmann::json submachines_json = nlohmann::json::object();
        for (auto const& [submachine_name, submachine_state] : state.submachines) {
            submachines_json[submachine_name] = build_fsm_state_json(submachine_state);
        }
        json["submachines"] = std::move(submachines_json);
    }
    if (!state.sessions.empty()) {
        nlohmann::json sessions_json = nlohmann::json::array();
        for (auto const& session_state : state.sessions) {
            sessions_json.push_back(build_fsm_state_json(session_state));
        }
        json["sessions"] = std::move(sessions_json);
    }

    return json;
}

std::string serialize_fsm_state(FsmTelemetryState const& state) {
    return build_fsm_state_json(state).dump(4);
}
...
struct FsmStateVisitor {
    std::vector<FsmTelemetryState> stack;
    ...
    FsmTelemetryState const& get_result() const {
        return stack.front();
    }
};
    ...
    if (print or telemetry) {
        ...
        if (current_signature != last_signature) {
            ...
            if (telemetry) {
                ...
                ctx.telemetry("FSM", "state", serialize_fsm_state(fsm_state_visitor.get_result()));
