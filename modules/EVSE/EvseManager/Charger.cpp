...
void Charger::process_cp_events_state(CPEvent cp_event) {
    ...
    case EvseState::StoppingCharging:
        if (cp_event == CPEvent::BCDtoEF and shared_context.reinit_requested) {
...
void Charger::setup(bool has_ventilation, const ChargeMode _charge_mode, bool _ac_hlc_enabled,
                    ...
                    const int reinit_duration_ms, const std::string reinit_method, const bool fail_on_powermeter_errors,
                    const bool raise_mrec9, const int sleep_before_enabling_pwm_hlc_mode_ms,
                    const utils::SessionIdType session_id_type, const int hlc_charge_loop_without_energy_timeout_s) {
